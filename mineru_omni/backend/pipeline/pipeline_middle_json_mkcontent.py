...
from mineru_omni.utils.enum_class import BlockType, ContentType, MakeMode
from mineru_omni.utils.language import detect_lang
...
def make_blocks_to_content_list_omni(para_block, img_buket_path, page_idx, page_size):
    def get_bbox(para: dict):
        if 'bbox_fs' in para.keys():
            return para['bbox_fs']
        elif 'bbox' in para.keys():
            return para['bbox']
        else:
            print(para)
            raise Exception('bbox not found')
    ...
    if para_type in [BlockType.TEXT, BlockType.LIST, BlockType.INDEX]:
        ...
        para_content = {'type': ContentType.IMAGE, 'img_path': '', 'bbox': get_bbox(para_block), 'page_size': page_size, BlockType.IMAGE_CAPTION: [], BlockType.IMAGE_FOOTNOTE: []}
        for block in para_block['blocks']:
            if block['type'] == BlockType.IMAGE_BODY:
                for line in block['lines']:
                    for span in line['spans']:
        ...
        para_content = {'type': ContentType.TABLE, 'img_path': '', 'bbox': get_bbox(para_block), 'page_size': page_size, BlockType.TABLE_CAPTION: [], BlockType.TABLE_FOOTNOTE: []}
        for block in para_block['blocks']:
            if block['type'] == BlockType.TABLE_BODY:
                for line in block['lines']:
                    for span in line['spans']:
