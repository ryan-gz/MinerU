...
def atom_model_init(model_name: str, **kwargs):
    ...
    if model_name == AtomicModel.Layout:
        atom_model = doclayout_yolo_model_init(kwargs.get("doclayout_yolo_weights"), kwargs.get("device"))
    elif model_name == AtomicModel.MFD:
        atom_model = mfd_model_init(kwargs.get("mfd_weights"), kwargs.get("device"))
    ...
    def __init__(self, **kwargs):
        ...
        if self.apply_formula:
            ...
            mfr_weight_dir = os.path.join(
                auto_download_and_get_model_root_path(ModelPath.unimernet_small), ModelPath.unimernet_small
            )
        ...
        if self.is_ppt:
            self.layout_model = atom_model_manager.get_atom_model(
                ...
                doclayout_yolo_weights=str(
                    os.path.join(
                        auto_download_and_get_model_root_path(ModelPath.doclayout_yolo), ModelPath.doclayout_yolo
                    )
                ),
                device=self.device,
            )
            ...
            self.layout_model = atom_model_manager.get_atom_model(
                ...
                doclayout_yolo_weights=str(
                    os.path.join(
                        auto_download_and_get_model_root_path(ModelPath.doclayout_yolo_ppt),
                        ModelPath.doclayout_yolo_ppt,
                    )
                ),
                device=self.device,
            )
